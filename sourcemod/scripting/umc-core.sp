...
ExtendMap(Handle:vM)
{
    // Edited by VIORA for Uncletopia to make the extend map vote reset the map
    // instead, otherwise strange things happened (very bad!)
    // Don't ask me anything about how this works. SourcePawn is an abomination
    // and fucking witchcraft.
    ...
    // Change the map, but to the current one
    DoMapChange(change_map_when, INVALID_HANDLE, mapname, "", "");
}
