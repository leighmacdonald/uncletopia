...
void BanUser(int userid, char[] reason, char[] pubreason)
{
    ...
    if (isAuthed)
    {
        ...
        if (GBANS && !GB_BanClient(-1, Cl, cheating, "0", BSBanned, demoname, demotick))
        {
            // Cheating bans are always considered permanent at the moment so "0" is the only valid duration for now.
            return;
        }
