...
{
	name        = "Reserved Slots+",
	author      = "AlliedModders LLC, viora",
...
/* Used for preventing users from being kicked between maps if someone is using a reserved slot */
ConVar    sm_reserve_grace;
ArrayList g_previouslyConnectedUsers;
...
public void OnClientPostAdminCheck(int client)
{
	...
	if (reserved > 0)
	{
		...
		if (type == 0)
		{
			if (clients <= limit || IsFakeClient(client) || flags & ADMFLAG_ROOT || flags & ADMFLAG_RESERVATION || IsTemporarilyReserved(client))
			{
...
bool IsTemporarilyReserved(int client)
{
...
public Action ClearTemporarySlotReserve()
{
	if (g_previouslyConnectedUsers != null)
	{
		g_previouslyConnectedUsers.Clear();
	}
	else {
		g_previouslyConnectedUsers = new ArrayList();
	}
